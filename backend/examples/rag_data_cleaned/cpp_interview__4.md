...
线程大概93年出现的，有SUN Solaris操作系统使用的线程叫做**UNIX International线程**，现在一直用的POSIX线程（POSIX threads），Pthreads线程的头文件是``，Win32线程是Windows API的一部分。
...
- **信号量**（同步原语，并不叫IPC）
  ...
  **信号量表示资源的数量。** **P操作** 会把信号量-1，-1之后如果信号量的值= 0，表明进程可以正常执行。**V操作**跟P操作正好相反。
		...
		long eip,long cs,long eflags,long esp,long ss)
{
	...
	*p = *current;	/* NOTE! this doesn't copy the supervisor stack */
    ...
    // CPU自动加载。最后返回新进程号。
	set_tss_desc(gdt+(nrtss));
	set_ldt_desc(gdt+(nrldt));
