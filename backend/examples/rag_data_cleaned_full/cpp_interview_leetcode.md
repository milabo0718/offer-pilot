...
- 仓库: cpp_interview
- 文件: leetcode刷题.md
- 许可: README 标注 MIT（仓库未见 LICENSE 文件）
...
默认compare策略是less。因为默认是大顶堆，首先输出最大元素，所以最开始来的元素最后才输出。记住大顶堆比较策略是`std::less`，小顶堆的比较策略是`std::greater`
...
TreeNode* constructMaximumBinaryTree(vector& nums) {
    ...
    TreeNode* build_binary_tree(vector& nums, int left, int right){
        //递归结束条件
        if(left >= right){
            return nullptr;
        }
...
TreeNode* buildTree(vector& preorder, vector& inorder) {
    ...
    TreeNode* build_tree_instance(vector& preorder, int pre_left, int pre_right,
    vector& inorder, int in_left, int in_right){
        if(in_left >= in_right){
            return nullptr;
        }
        int left_count = 0;
...
TreeNode* buildTree(vector& inorder, vector& postorder) {
    ...
    TreeNode* bulid_tree_instance(vector& inorder, int in_left, int in_right,
    vector& postorder, int post_left, int post_right){
        if(in_left >= in_right){
            return nullptr;
        }
        int index = 0;
...
public:
    ...
    vector construct(int left, int right){
        vector tmp;
        if(left > right){
        ...
        for(int i = left; i <= right; i++){
            vector left_vec = construct(left, i-1);
            vector right_vec = construct(i+1, right);
...
void sortColors(vector& nums) {
    ...
    void quciksort(vector& nums, int l, int r){
        if(l >= r){
            return;
        }
...
int maximumGap(vector& nums) {
    ...
    int partion(vector& nums, int l, int r){
        int pivot = nums[l];
        int pivot_index = l;
        while(l < r){
            while(l = pivot){
...
- 代码
  ...
  int findTargetSumWays(vector& nums, int target) {
      ...
      vector> dp(len+1, vector(goal+1, 0));
...
vector count_one_zero(string& strs){
    vector dp(2, 0);
    int n = strs.size();
    for(int i = 0; i < n; i++){
        if(strs[i] == '0'){
...
int findMaxForm(vector& strs, int m, int n) {
    int len = strs.size();
    vector>> dp(len+1,vector>(m+1,vector(n+1, 0)));
    for(int i = 1;i < len + 1; i++){
        vector count_arr = count_one_zero(strs[i -1]);
...
public:
    int coinChange(vector& coins, int amount) {
        int len = coins.size();
        vector> dp(len+1, vector(amount+1, 10001));
...
int change(int amount, vector& coins) {
    int len = coins.size();
    vector> dp(len+1, vector(amount+1, 0));
    dp[0][0] = 1;
    for(int i = 1; i < len + 1; i++){
        for(int j = 0; j < amount+1; j++){
            if(j-coins[i-1] >= 0){
...
int rob(vector& nums) {
        ...
        vector nums_temp1(n-1);
        vector nums_temp2(n-1);
        for(int i = 0; i < n - 1; i++){
    ...
    int rob_with_nums(vector& nums_temp){
...
int maxProfit(vector& prices) {
        int n = prices.size();
        vector>> dp(n+1,vector>(3, vector(2, 0)));
        for(int k = 0; k < 3; k++){
            dp[1][k][0] = 0;
            dp[1][k][1] = -prices[0];
        }
...
int sliding(vector& short_vec, vector& long_vec){
...
int get_repeat(vector& short_vec, int i, vector& long_vec, int j, int common_len){
    int max_repeat = 0;
    int count = 0;
...
int lengthOfLIS(vector& nums) {
    ...
    sort(dp.begin(), dp.end(), std::greater());
    return dp[0];
}
```
...
- 代码
  ...
  ```c++
  bool static cmp(vector& v1, vector& v2){
...
```c++
    ...
    void back_tracing(vector& nums, vector& use_check){
...
vector> res;
    ...
    vector> permuteUnique(vector& nums) {
        vector user_check_vec(nums.size(), false);
        sort(nums.begin(), nums.end());
        back_tracing(nums, user_check_vec);
        return res;
    }
    void back_tracing(vector& nums, vector vec){
        if(temp.size() == nums.size()){
            res.push_back(temp);
            return;
        }
...
```c++
    ...
    vector> solveNQueens(int n) {
        vector chess(n, string(n, '.'));
        back_tracing(chess, 0, n);
        return res;
    }
    void back_tracing(vector& chess, int row, int n ){
        if(row == n){
            res.push_back(chess);
            return;
        }
    ...
    bool is_suit(vector& chess, int row, int col, int n){
        //检查该列是否有冲突
        for(int i = 0; i < row; i++ ){
            if(chess[i][col] == 'Q'){
                return false;
...
- c++知识点
  - `vector chess(n, string(n, '.'))`这是初始化的一种
...
bool back_tracing(vector>& board){
...
bool is_suit(vector>& board, int col, int row, char k){
    //判断行
    for(int i = 0; i < 9; i++){
        if(board[row][i] == k){
            return false;
...
int get_mine(vector>& board, int x, int y){
    int count = 0;
    //上
...
void back_tracing(unordered_map& m_map, string digits, int index){
...
bool dfs(vector>& board, string word, int row, int col, int index){
    //不匹配就不要浪费时间直接返回
...
void dfs(vector>& image, int row, int col, int newColor, int flag){
...
vector> pacificAtlantic(vector>& heights) {
    ...
    vector> P(row, vector(col, false));
    vector> A(row, vector(col, false));
    for(int i = 0; i < row; i++){
        dfs(heights, P, i , 0);
...
void dfs(vector>& heights, vector>& ocean, int row, int col){
    if(ocean[row][col]){
        return;
    }
    int m = ocean.size();
...
public:
    ...
    void dfs(vector>& grid, int row, int col, int& max_area, int tmp_area){
        if(grid[row][col] == 0){
            return;
        }
...
bool dfs(vector>& grid, int row, int col){
    int m = grid.size();
    int n = grid[0].size();
    //判断是否在边界
    if(row = m || col  = n ){
...
void dfs(vector>& grid1, vector>& grid2, int row, int col, bool& flag){
    if(grid2[row][col] == 0){
        return;
    }
    //表明grid2中的岛屿不在grid1中
...
1. `vector& start1`: (x1,y1)
2. `vector& end1`: (x2,y2)
3. `vector& start2` :(x3,y3)
4. `vector& end2` :(x4,y4)
