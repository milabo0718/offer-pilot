...
void* Thread_pool::manager(void* arg){
    Thread_pool* pool = static_cast(arg);
    while(pool->shutdown){
        //每五秒检测一次
        sleep(5);
        ...
        if(queue_size > live_size && live_size m_max_num){
            pthread_mutex_lock(&pool->m_mutex_pool);
            int count = 0;
            for(int i = 0; i m_max_num && count < NUMBER
                && pool->m_live_num m_max_num; i++){
                    //之前memset了
...
```c++
void heap_bulid(vector& vec, int root, int len)
	int left_child = root*2 + 1；
	int righ_child = root*2 + 2；
	int max_root = root;
	if(left_child  vec[max_root]){
		max_root = left;
	}
	if(right_child  vec[max_root]){
		max_root = right;
	}
...
```c++
void merge(vector& vec, vector& tmp1, vector& tmp2){
    int len1 = tmp1.size();
    int len2 = tmp2.size();
    int p1 = 0;
    int p2 = 0;
...
void mergesort(vector& vec){
    ...
    vector tmp1(vec.begin(), mid);
    vector tmp2(mid, vec.end());
    mergesort(tmp1);
    mergesort(tmp2);
    vec.clear();
    merge(vec, tmp1, tmp2);
...
/******普通二叉树*******/
BTree* create_tree(vector& node, int index = 0){
    int len = node.size();
    if(node[index] == 0){
        return nullptr;
    }
...
void split(const std::string& str, std::vector* ret_, std::string sep = ",")
{
    if (str.empty()) {
        return;
    }
...
bool CheckDateValid(const std::string& strDate)
{
    ...
    if(strPureDate.length()  10) {
        return false;
    }

    std::vector vecFields;
